...
func main() {
	...
	if len(os.Args) < 2 {
		fmt.Println("Usage: book <book_name>")
		return
	}

	searchTerm := strings.ToLower(os.Args[1])
	dir := "/mnt/hdd/books/it"
...
func launchBook(book string) {
	fmt.Printf("Launching book: %s\n", book)
	cmd := exec.Command("deepin-reader", book)
	err := cmd.Start()
	if err != nil {
		fmt.Printf("Failed to launch book: %v\n", err)
	}
}
